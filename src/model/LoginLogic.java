...
public final class LoginLogic {
	...
	/**
	 * 第一引数、第二引数に与えられたデータが、プロジェクト内に保存してあるadmin.propertiesの内容と一致しているか確認するメソッドです。<br/>
	 * admin.propertiesには キー値＝値 の形式でデータが保存されています。
	 * 第一引数、第二引数それぞれがID、パスワードと一致した場合true、そうでない場合falseを返します。
	 * @param id ログインID
